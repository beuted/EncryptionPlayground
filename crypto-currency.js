...
class User {
    ...
    <ul>
        ...
        <li class="localBlockChain hover-to-see">localBlockChain (${this.localBlockChain.length}): <code>${JSON.stringify(this.localBlockChain, undefined, 2)}</code></li>
...
`;
    ...
    VerifySignedMessageAndAddToBlockChain(signedMessage) {
        ...
        let isValid = this.VerifySignedMessage(signedMessage)
        if (!isValid) {
            return false;
        }
